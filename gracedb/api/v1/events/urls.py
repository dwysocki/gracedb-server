...
urlpatterns = [
    ...
    re_path(r'^(?P<graceid>[GEHMT]\d+)$', never_cache(EventDetail.as_view()),
        name='event-detail'),
    re_path(r'^(?P<graceid>[GEHMT]\d+)/update-grbevent/$',
        never_cache(GrbEventPatchView.as_view()), name='update-grbevent'),

    # Event Log Resources
    # events/{graceid}/logs/[{logid}]
    re_path(r'^(?P<graceid>[GEHMT]\d+)/log/$', never_cache(EventLogList.as_view()),
        name='eventlog-list'),
    re_path(r'^(?P<graceid>[GEHMT]\d+)/log/(?P<n>\d+)$',
        never_cache(EventLogDetail.as_view()), name='eventlog-detail'),

    # VOEvent Resources
    # events/{graceid}/voevent/[{serial_number}]
    re_path(r'^(?P<graceid>[GEHMT]\d+)/voevent/$', never_cache(VOEventList.as_view()),
        name='voevent-list'),
    re_path(r'^(?P<graceid>[GEHMT]\d+)/voevent/(?P<n>\d+)$',
        never_cache(VOEventDetail.as_view()), name='voevent-detail'),

    # EMBB Resources
    # events/{graceid}/logs/[{logid}]
    re_path(r'^(?P<graceid>[GEHMT]\d+)/embb/$', never_cache(EMBBEventLogList.as_view()),
        name='embbeventlog-list'),
    re_path(r'^(?P<graceid>[GEHMT]\d+)/embb/(?P<n>\d+)$',
        never_cache(EMBBEventLogDetail.as_view()), name='embbeventlog-detail'),
    re_path(r'^(?P<graceid>[GEHMT]\d+)/emobservation/$',
        never_cache(EMObservationList.as_view()), name='emobservation-list'),
    re_path(r'^(?P<graceid>[GEHMT]\d+)/emobservation/(?P<n>\d+)$',
        never_cache(EMObservationDetail.as_view()), name='emobservation-detail'),
#    re_path(r'(?P<graceid>[GEHMT]\d+)/emobservation/(?P<n>\d+)/emfootprint/$',
#        EMFootprintList.as_view(), name='emfootprint-list'),
#    re_path(r'(?P<graceid>[GEHMT]\d+)/emobservation/(?P<n>\d+)/emfootprint/(?P<m>\d+)$',
#        EMFootprintDetail.as_view(), name='emfootprint-detail'),

    # Tag Resources
    re_path(r'^(?P<graceid>[GEHMT]\d+)/tag/$', never_cache(EventTagList.as_view()),
        name='eventtag-list'),
    re_path(r'^(?P<graceid>[GEHMT]\d+)/tag/(?P<tagname>.+)$',
        never_cache(EventTagDetail.as_view()), name='eventtag-detail'),
    re_path(r'^(?P<graceid>[GEHMT]\d+)/log/(?P<n>\d+)/tag/$',
        never_cache(EventLogTagList.as_view()), name='eventlogtag-list'),
    re_path(r'^(?P<graceid>[GEHMT]\d+)/log/(?P<n>\d+)/tag/(?P<tagname>.+)$',
        never_cache(EventLogTagDetail.as_view()), name='eventlogtag-detail'),

    # Permission Resources
    re_path(r'^(?P<graceid>[GEHMT]\d+)/perms/$',
        never_cache(EventPermissionList.as_view()), name='eventpermission-list'),
    re_path(r'^(?P<graceid>[GEHMT]\d+)/perms/(?P<group_name>.+)/$', 
        never_cache(GroupEventPermissionList.as_view()), name='groupeventpermission-list'),
    re_path(r'^(?P<graceid>[GEHMT]\d+)/perms/(?P<group_name>.+)/(?P<perm_shortname>\w+)$', 
    ...
    re_path(r'^(?P<graceid>[GEHMT]\d+)/signoff/$',
