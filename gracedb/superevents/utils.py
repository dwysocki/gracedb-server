...
def confirm_superevent_as_gw(superevent, user, gw_id, add_log_message=True,
    ...
    superevent.confirm_as_gw(user, gw_id)

    # Create log message
    gw_log = None
    if add_log_message:
        message = ("Confirmed as a gravitational wave: ID changed from "
            "{old} -> {new}. Nickname {new} automatically assigned.").format(old=old_id, new=superevent.superevent_id)
...
# Create a new nickname for the superevent. Write a log message. 

def create_nickname(superevent, user, nickname, comment):
    if nickname_exists(nickname):
        return HttpResponseNotAllowed('Cannot assign name. Nickname {} exists.')

    # Create the nickname:
    nn = Nickname(content_object=superevent,
                  name=nickname,
                  comment=comment,
                  creator=user)
    nn.save()
    ...
    # Write a comment:
    log_comment = 'Created nickname "{nn}" with comment "{comm}"'.format(nn=nickname, comm=comment)
    nickname_log = create_log(user, log_comment, superevent, issue_alert=False)
